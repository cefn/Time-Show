	...
	class Tv implements Traveller.Listener, Tunable, Drawable{
		...
		public void setPlaying(GSMovie next){
			...
			if(playing != null){
				...
				if(duration != 0){
					...
					playing.jump(((float)timecode) * 0.99f / 1000f);					
				}
				*/
				playing.volume(0.9);
			}
		...
		public synchronized void draw(PApplet applet) {
			...
			*/
			...
			if(loopcount % 2==1){
				//noise.volume(0.5 * getNoiseVolume());				
			}
			applet.tint(255,255);
		...
		double lastMovieVolume;
		public double getMovieVolume() {
			double value = constrain(1.0f - getBandOffset(), 0.01f, 0.99f);
			//if(value != lastMovieVolume){System.out.println("m:" + value);}
			lastMovieVolume = value;
			return lastMovieVolume;
		}
		
		double lastNoiseVolume;
		public double getNoiseVolume() {
			double value = constrain(getBandOffset(), 0.01f, 0.99f);
			//if(value != lastNoiseVolume){System.out.println("n:" + value);}
			lastNoiseVolume = value;
			return lastNoiseVolume;
		}
